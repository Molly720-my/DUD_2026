...
def get_paths(path) -> Dict[str,str]:
    ...
    if "masks" in path:
        ...
        if "desobav2" in path:
            return {
                "gt_path": os.path.join(
                    *parts[:-2], "real_images", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
                ),
                "mask_path": path,
                "image_path": os.path.join(
                    *parts[:-2], "composite_images", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
                ),
                "shadow_mask_path": os.path.join(
                    *parts[:-2], "shadow_masks", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
                ),
            }
        else:
            return {
                ...
                "mask_path": path,
                "image_path": os.path.join(
                    *parts[:-2], "composite_images", f"{img_name_parts[0]}.jpg"
                ),
            }
    elif "composite" in path:
        # print("if composite path:", path)
        # print("if composite parts:", *parts[:-2])
        # print("if composite name:", img_name_parts[0], "$$$$", img_name_parts[1])
        if "HCOCO" in path or "HAdobe5k" in path or "Hday2night" in path or "HFlickr" in path:
            return {
            "gt_path": os.path.join(
                *parts[:-2], "real_images", f"{img_name_parts[0]}.jpg"
            ),
        ...
        elif "desobav2" in path:
            # NOTE debug XXX
            # print("gt_path:", os.path.join(
            #         *parts[:-2], "real_images", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
            #     ))
            # print("mask_path:", os.path.join(
            #         *parts[:-2], "masks", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
            #     ))
            # print("image_path:", path)
            # print("shadow_mask_path:", os.path.join(
            #         *parts[:-2], "shadow_masks", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
            #     ))
            
            return {
                # NOTE Inpainting Stage1: add '_{img_name_parts[1]}'
                "gt_path": os.path.join(
                    *parts[:-2], "real_images", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
                ),
                # NOTE Inpainting Stage1: 'png' -> 'jpg'
                "mask_path": os.path.join(
                    *parts[:-2], "masks", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
                ),
                "image_path": path,
                # NOTE Inpainting Stage1: add shadow mask path XXX 'png' -> 'jpg'
                "shadow_mask_path": os.path.join(
                    *parts[:-2], "shadow_masks", f"{img_name_parts[0]}_{img_name_parts[1]}.jpg"
                ),
            }
...
class IhdDatasetMultiRes(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print("mask path:", paths["mask_path"])
        mask = Image.open(paths["mask_path"]).convert("1")
        # NOTE Inpainting Stage1: add shadow mask path
        shadow_mask = Image.open(paths["shadow_mask_path"]).convert("1")
        ...
        if self.random_flip and np.random.rand() > 0.5 and self.split == "train":
            ...
            shadow_mask = TF.hflip(shadow_mask)
        if self.random_crop:
            for _ in range(5):
            ...
            # NOTE Inpainting Stage1: 
            for _ in range(5):
                shadow_mask_tensor = TF.to_tensor(shadow_mask)
                crop_box = T.RandomResizedCrop.get_params(
                    shadow_mask_tensor, scale=[0.5, 1.0], ratio=[3 / 4, 4 / 3]
                )
                cropped_shadow_mask_tensor = TF.crop(shadow_mask_tensor, *crop_box)
                h, w = cropped_shadow_mask_tensor.shape[-2:]
                if cropped_shadow_mask_tensor.sum() < 0.01 * h * w:
                    continue
                break
        ...
        for resolution in self.resolutions:
            if self.random_crop:
                ...
                # NOTE Inpainting Stage1:
                this_res_shadow_mask = TF.resize(
                    TF.crop(shadow_mask, *crop_box),
                    size=(resolution, resolution),
                )  # default : bilinear resample ; antialias for PIL Image
            else:
                if comp.size == (resolution, resolution):
                    ...
                    this_res_real = real
                    # NOTE Inpainting Stage1:
                    this_res_shadow_mask = shadow_mask
                else:
                    this_res_comp = TF.resize(
                    ...
                    # NOTE Inpainting Stage1:
                    this_res_shadow_mask = TF.resize(
                        shadow_mask, [resolution, resolution]
                    )  # default : bilinear resample ; antialias for PIL Image
            ...
            # NOTE Inpainting Stage1:
            this_res_shadow_mask = TF.to_tensor(this_res_shadow_mask)
            this_res_shadow_mask = (this_res_shadow_mask >= 0.5).float()  # mask : tensor , 0/1
            this_res_shadow_mask = self.dilate_mask_image(this_res_shadow_mask)
            ...
            example[resolution] = {
                ...
                # NOTE Inpainting Stage1:
                "shadow_mask": this_res_shadow_mask,
                "shadow_mask_path": paths["shadow_mask_path"],
