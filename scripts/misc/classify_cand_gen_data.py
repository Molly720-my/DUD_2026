...
class CustomDataset(Dataset):
    def __init__(self, dataset_root):
        ...
        for cand_composite_path in tqdm(cand_composite_paths):
            # NOTE SG: split("_")[0] -> split("_")[:2]
            image_name = os.path.basename(cand_composite_path).split("_")[:2]
            # NOTE SG: 
...
if __name__ == "__main__":
    ...
    for step, batch in enumerate(dataloader):
        ...
        for i, (p, idx) in enumerate(zip(paths, max_index)):
            os.system(
                # NOTE: 'composite_images' -> 'sort_composite_images'
                f"cp {p[idx.item()]} {os.path.join(dataset_root, 'sort_composite_images')}"
            )
        # NOTE update progress_bar:
