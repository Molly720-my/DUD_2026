...
# NOTE: pipeline_stable_diffusion_harmony -> pipeline_SGH_adapter
from src.pipelines.pipeline_SGH_adapter import (
    StableDiffusionHarmonyPipeline,
)

# NOTE Inpainting Stage1:
from src.models.adapter import T2IAdapter
...
def parse_args():
    ...
    # NOTE Inpainting Stage1:
    parser.add_argument(
        "--load_Inpainting_pretrained_weights",
        type=str,
        default=None,
    )
    ...
    # NOTE Inpainting Stage1: adapter weight path
    parser.add_argument(
...
def main():
    ...
    accelerator = Accelerator(
        ...
        # 指定在更新模型参数之前要累计多少步的梯度。这对于处理大批量数据非常有用。
        mixed_precision=args.mixed_precision,
        log_with=args.report_to,
        # 指定日志记录的方式，可以是 'tensorboard'、'wandb' 等。可以通过传递列表来同时记录到多个地方。
        project_dir=project_dir,
        # 指定项目的目录，通常用于保存模型和日志        
        kwargs_handlers=[ddp_kwargs],
        # 可以接收一个可调用对象的列表，这些对象在处理 Accelerator 的关键字参数时会被调用。
    )
    ...
    # NOTE Inpainting Stage1:
    noise_Inpainting_scheduler = DDPMScheduler.from_pretrained(
    ...
    # NOTE Inpainting Stage1:
    if args.load_Inpainting_pretrained_weights != None:
        unet_Inpainting_path = args.load_Inpainting_pretrained_weights
    else:
        unet_Inpainting_path = os.path.join(args.pretrained_model_name_or_path, "unet_Inpainting")
    unet_Inpainting = UNet2DConditionModel.from_pretrained(
        unet_Inpainting_path,
    )
    ...
    '''NOTE Inpainting Stage1: load adapter weight BEGIN''' 
    ...
    adapter = adapter.from_pretrained(
        args.pretrained_adapter_model_name_or_path, torch_dtype=weight_dtype, varient="fp16"
    ).to("cuda")
    
    # adapter = T2IAdapter.from_pretrained(
    #     args.pretrained_adapter_model_name_or_path, torch_dtype=weight_dtype, varient="fp16"
    # ).to("cuda")
    '''NOTE Inpainting Stage1: load adapter weight END'''

    # Freeze vae and text_encoder
    vae.requires_grad_(False)
    text_encoder.requires_grad_(False)
    # NOTE Inpainting Stage1: unet and adapter
    unet_Inpainting.requires_grad_(False)
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            # NOTE Inpainting Stage1:
            unet_Inpainting.enable_xformers_memory_efficient_attention()
    ...
    # NOTE Inpainting Stage1:
    model_Inpainting = unet_Inpainting
    model = unet
    ...
    # NOTE Inpainting Stage1: unet_Inpainting and adapter
    unet_Inpainting.to(accelerator.device, dtype=weight_dtype)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                with accelerator.autocast():
                    ...
                    if args.mode == "normal":
                        ...
                        # NOTE Inpainting Stage1:
                        cond_latents_Inpainting = torch.cat([shadow_mask_64, comp_latents], dim=1)
                    else:
                    ...
                    # NOTE Inpainting Stage1:
                    noisy_Inpainting_latents = noise_Inpainting_scheduler.add_noise(latents, noise, timesteps)
                    latent_model_Inpainting_input = torch.cat(
                        [noisy_Inpainting_latents, cond_latents_Inpainting], dim=1
                    )

                    encoder_hidden_states = text_encoder(caption_ids)[0]

                    '''NOTE Inpainting Stage1: Adapter processing XXX BEGIN''' 
                    masked_image = comp.to(device=accelerator.device)
                    shadow_mask_adapter = shadow_mask_adapter.to(device=accelerator.device)
                    adapter_input = torch.cat([masked_image, shadow_mask_adapter], dim=1)
                    adapter_features = adapter(adapter_input)
                    '''NOTE Inpainting Stage1: Adapter processing XXX END''' 

                    # NOTE Inpainting Stage1:
                    model_pred_Inpainting = model_Inpainting(
                    ...
                    ''' NOTE 将Inpainting输出的预测噪声, 对加噪后的latents进行降噪, 重新获取Inpainting real: BEGIN'''
                    # NOTE random sample timesteps to step and denoise:
                    # latents = noise_Inpainting_scheduler.step(model_pred_Inpainting, timesteps[0].item(), noisy_Inpainting_latents).prev_sample
                    latents_stage1 = []
                    for i, t in enumerate(timesteps):
                        latents_stage1_cut = noise_Inpainting_scheduler.step(model_pred_Inpainting, t.item(), noisy_Inpainting_latents).prev_sample
                        latents_stage1.append(latents_stage1_cut[i:i+1])
                    latents = torch.cat(latents_stage1, dim=0)
                    # print("latents.Shape:", latents.shape)
                    ''' NOTE 将Inpainting输出的预测噪声, 对加噪后的latents进行降噪, 重新获取Inpainting real: END'''

                    #=========================================================================================#
                    noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
                    
                    # NOTE Harmony Stage2: XXX 重点：cond的问题，cond = 'cond_latents' or 'latents' XXX
                    temp = latents
                    cond_latents = torch.cat([mask, temp], dim=1)
                    ...
                    # epsilon 通常表示在扩散过程中添加到原始数据中的噪声。
                    # NOTE Harmony Stage2: noise -> model_pred_Inpainting (not good enough)
                        target = noise
                    elif noise_scheduler.config.prediction_type == "v_prediction":
                    # v_prediction 表示模型对噪声的预测，通常是指模型在特定时间步长下对 epsilon 的估计。
                    # NOTE Harmony Stage2: noise -> model_pred_Inpainting (not good enough)
                    ...
                    if args.nofg:
                        ...
                        loss_shadow = loss_shadow * loss_scale_shadow
                        loss_shadow = loss_shadow.mean() * args.lambda_loss_others
                        ...
                        total_loss = loss + loss_shadow + loss_cos
            ...
            if accelerator.sync_gradients:
                ...
                if (
                    ...
                    # NOTE Inpainting Stage1: shadow_mask
