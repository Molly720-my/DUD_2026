...
# NOTE Inpainting Stage1: extra import ControlNetModel
...
from src.pipelines.pipeline_inpainting_controlnet import (
    StableDiffusionHarmonyPipeline,
)
# NOTE Inpainting Stage1:
# from src.models.adapter import T2IAdapter, Adapter
# NOTE loss: import perceptual loss
from src.loss.loss import PerceptualLoss
...
def parse_args():
    ...
    # NOTE Inpainting Stage1: controlnet ckpt path
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="ratio for foreground mask local loss",
    )
    parser.add_argument(
        "--lambda_loss_per",
        type=float,
        default=0.2,
        help="ratio for whole loss",
    )
    parser.add_argument(
        "--lambda_loss_cos",
        type=float,
        default=1.0,
        help="ratio for other loss",
    )
...
def main():
    ...
    # NOTE Inpainting Stage:
    noise_scheduler = DDPMScheduler.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="scheduler_Inpainting"
    )
    ...
    if args.load_pretrained_weights != None:
        ...
        unet_path = os.path.join(args.pretrained_model_name_or_path, "unet_Inpainting")
    accelerator.print(f"using unet from <<{unet_path}>>")
    unet = UNet2DConditionModel.from_pretrained(
        unet_path,
    )

    # controlnet = ControlNetModel.register_to_config(conditioning_channels=4)
    if args.controlnet_model_name_or_path:
        logger.info("Loading existing controlnet weights")
        controlnet = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path, conditioning_channels=1)
    else:
    ...
    # NOTE Inpainting Stage1: “unet” -> “controlnet”
    model = controlnet
    model.train()
    ...
    # NOTE loss: perceptual loss init
    perLoss = PerceptualLoss(device=accelerator.device)
    # perLoss = accelerator.prepare(perLoss)

    for epoch in range(first_epoch, args.num_train_epochs):
        # NOTE Inpainting Stage1: add unet.train()
        # unet.train()
        # model.train()
        # NOTE per loss: add vae.train()
        # vae.train()
        train_loss = 0.0
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                with accelerator.autocast():
                    ...
                    # print('@@ONE@@ shadow_mask.shape:', shadow_mask.shape)
                    ...
                    if args.resolution != args.infer_resolution:
                        ...
                        mask = (mask > 0.5).float()
                        # NOTE Inpainting Stage1:
                        shadow_mask = TF.resize(
                            shadow_mask,
                            size=tgt_size,
                            interpolation=InterpolationMode.BILINEAR,
                            antialias=True,
                        ).clamp(0,1)
                        shadow_mask = (shadow_mask > 0.5).float()

                        # NOTE Inpainting Stage1:
                        # comp = comp * (shadow_mask < 0.5)
                    # print('@@TWO@@ shadow_mask.shape:', shadow_mask.shape)
                    ...
                    # NOTE Inpainting Stage1: mask -> mask_64
                    ...
                    # NOTE Inpainting Stage1: 
                    shadow_mask_64 = torch.nn.functional.interpolate(
                        shadow_mask,
                        size=(
                            comp.shape[2] // vae_scale_factor,
                            comp.shape[3] // vae_scale_factor,
                    ...
                    if args.mode == "normal":
                        ...
                        cond_latents = torch.cat([shadow_mask_64, comp_latents], dim=1)
                    else:
                        latents = comp_latents
                        cond_latents = torch.cat([mask_64, real_latents], dim=1)
                    ...
                    '''NOTE Inpainting Stage1: controlnet XXX BEGIN XXX'''
                    # controlnet_image = torch.cat([shadow_mask, comp], dim=1)
                    controlnet_image = comp * (shadow_mask < 0.5)
                    # shadow_mask = shadow_mask.expand(comp.shape)

                    down_block_res_samples, mid_block_res_sample = model(
                        latent_model_input,
                        timesteps,
                        encoder_hidden_states=encoder_hidden_states,
                        controlnet_cond=shadow_mask,
                        return_dict=False,
                    )
                    '''NOTE Inpainting Stage1: controlnet XXX END XXX'''
 
                    # NOTE Inpainting Stage1: model -> unet, add down_block and mid_block, add return_dict=False
                    #                         .sample -> [0]
                    model_pred = unet(
                        latent_model_input, 
                        timesteps, 
                    ...
                    ''' NOTE per loss: prepare img to input VGG XXX BEGIN'''
                    # latents_step = []
                    # for i, t in enumerate(timesteps):
                    #     latents_stage1_cut = noise_scheduler.step(model_pred, t.item(), noisy_latents).prev_sample
                    #     latents_step.append(latents_stage1_cut[i:i+1])
                    # latents_step = torch.cat(latents_step, dim=0)

                    # from src.models.condition_vae import ConditionVAE
                    # cvae_cond = torch.cat([comp, mask], dim=1)
                    # if isinstance(vae, ConditionVAE):
                    #     image = vae.decode_with_cond(
                    #         latents_step / vae.config.scaling_factor, 
                    #         cvae_cond, 
                    #         return_dict=False, 
                    #         generator=(
                    ...
                    #     )[0]
                    # else:
                    #     image = vae.decode(latents_step / vae.config.scaling_factor, return_dict=False)[0]
                    # image = (image / 2 + 0.5).clamp(0, 1).cpu()
                    ''' NOTE per loss: prepare img to input VGG XXX END'''
                    ...
                    if args.nofg:
                        ...
                        # NOTE Dilate the shadow mask:
                        # import numpy as np
                        # import cv2
                        # 定义膨胀的结构元素
                        # kernel = np.ones((3, 3), np.uint8)
                        # 执行膨胀操作
                        # dilated_mask = cv2.dilate(shadow_mask_64, kernel, iterations=1)

                        # NOTE Inpainting Stage1: change all ‘mask’ to ‘shadow_mask_64’
                        loss = F.mse_loss(
                            model_pred.float() * shadow_mask_64,
                            target.float() * shadow_mask_64,
                            reduction="none",
                        ).mean(dim=(2, 3), keepdim=True)
                        loss_scale = (
                            (shadow_mask_64.shape[2] * shadow_mask_64.shape[3])
                            / shadow_mask_64.sum(dim=(2, 3), keepdim=True)
                        ).clamp(1, 5)
                        loss = loss * loss_scale
                        loss = loss.mean() * args.lambda_loss_local
                        
                        # NOTE loss: add whole mse loss
                        # loss_whole = F.mse_loss(
                        #     model_pred.float(), target.float(), reduction="mean"
                        # )
                        # loss_whole = loss_whole * args.lambda_loss_others
                        
                        # NOTE loss: add perceptual loss
                        # image = image.to(device=accelerator.device)
                        # image.requires_grad=True
                        # real = real.to(device=accelerator.device)
                        # loss_per = perLoss(image, real)
                        # loss_per_scale = (
                        #     (shadow_mask.shape[2] * shadow_mask.shape[3])
                        #     / shadow_mask.sum(dim=(2, 3), keepdim=True)
                        # ).clamp(1, 5)
                        # loss_per = (loss_per * loss_per_scale).mean()
                        # loss_per = loss_per * args.lambda_loss_per
                        
                        # NOTE loss: cosine similarity
                        loss_cos = torch.cosine_similarity(
                            model_pred.float() * shadow_mask_64, target.float() * shadow_mask_64,
                            )
                        loss_cos = (1- loss_cos).mean()
                        loss_cos = loss_cos * args.lambda_loss_cos
                        
                        ''' NOTE total loss: '''
                        total_loss = loss
                        # + loss_whole + loss_cos + loss_per + loss_cos


                # Gather the losses across all processes for logging (if we use distributed training).
                # NOTE loss: loss -> total_loss
            ...
            if accelerator.sync_gradients:
                ...
                # NOTE loss: add "perceptual_loss" or "whole_loss" to exhibit
                accelerator.log({"train_loss": train_loss, 
                                 # "perceptual_loss": loss_per.item(),
                                 # "whole_loss": loss_whole.item(),
                                 "cosine_loss": loss_cos.item(),
                                 "foreground_loss": loss.item(),
                                 }, step=global_step)
                train_loss = 0.0
                ...
                if (global_step % args.checkpointing_steps == 0) or (
                    ...
                    if accelerator.is_main_process:
                        # NOTE Inpainting Stage1: unwrap_model
                ...
                if (
                    ...
                    eval_unet=UNet2DConditionModel.from_pretrained(
                        args.pretrained_model_name_or_path,
                        # NOTE Inpainting Stage1: unet -> unet_Inpainting
                        subfolder="unet_Inpainting",
                        torch_dtype=weight_dtype,
                    )
                    
                    # NOTE Inpainting Stage1: controlnet 
                    eval_controlnet = ControlNetModel.from_unet(eval_unet, conditioning_channels=1)
                    eval_controlnet.to(accelerator.device, dtype=weight_dtype)

                    # NOTE Inpainting Stage1: eval_unet -> eval_adapter
                    if args.use_ema:
                        ema_model.copy_to(get_trainable_parameters(eval_controlnet))

                    pipeline = StableDiffusionHarmonyPipeline.from_pretrained(
                        args.pretrained_model_name_or_path,
                        unet=eval_unet,
                        # NOTE Inpainting Stage1: add controlnet
                        controlnet=eval_controlnet,
                    ...
                    # NOTE Inpainting Stage1:
                    eval_shadow_mask_images = eval_batch["shadow_mask"]
                    ...
                    if args.resolution != args.infer_resolution:
                        ...
                        # NOTE Inpainting Stage1:
                        eval_shadow_mask_images = TF.resize(
                            eval_shadow_mask_images,
                            size=tgt_size,
                            interpolation=InterpolationMode.BILINEAR,
                            antialias=True,
                        ).clamp(0,1)
                        eval_shadow_mask_images = (eval_shadow_mask_images > 0.5).float()
                    ...
                    #     nrow=nrow,
                    # )
                    # NOTE Inpainting Stage1: XXX test XXX
                    shadow_mask_to_save = make_grid(
                        eval_shadow_mask_images,
                        nrow=nrow,
                    )
                    ...
                    save_image(
                        shadow_mask_to_save,
                        os.path.join(image_logging_dir, f"s{global_step:08d}_shadow_mask.jpg"),
                    )
                    save_image(
                        sample_to_save,
