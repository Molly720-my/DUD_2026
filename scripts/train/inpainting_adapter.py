...
from src.pipelines.pipeline_inpainting_adapter import (
...
def parse_args():
    ...
    # NOTE Inpainting Stage1: adapter type
    parser.add_argument(
        "--adapter_type",
        type=str,
        default="Adapter",
        choices=["Adapter", "NoRes_Adapter", "DIY_NoRes_Adapter"],
    )
    parser.add_argument(
        "--lambda_loss_local",
        type=float,
...
def main():
    ...
    # NOTE Inpainting Stage1: load Adapter
    adapter = T2IAdapter(cin=int(64 * 4), 
    ...
    # NOTE Inpainting Stage1: “unet” -> “adapter”
    model = adapter
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                with accelerator.autocast():
                    ...
                    # print('@@THREE@@ shadow_mask.shape:', shadow_mask.shape)
                    adapter_input = torch.cat((comp, shadow_mask), dim=1)
                    adapter_features = model(adapter_input)
                    '''NOTE Inpainting Stage1: Adapter processing XXX END''' 
 
                    # NOTE Inpainting Stage1: model -> unet, add down_block_additional_residuals
                    model_pred = unet(
                        latent_model_input, 
                        timesteps, 
                        encoder_hidden_states,
                        down_intrablock_additional_residuals=[sample.to(dtype=weight_dtype) for sample in adapter_features]
                    ).sample
                    ...
                    # if latents_step.grad_fn is not None:
                    #     print("latents_step is in the computation graph.")
                    # else:
                    #     print("latents_step is NOT in the computation graph.")
                    ...
                    if args.nofg:
                        ...
                        # NOTE total loss:
                        total_loss = loss + loss_per + loss_cos
                        # + loss_whole + loss_cos
            ...
            if accelerator.sync_gradients:
                ...
                if (
                    ...
                    # NOTE Inpainting Stage1:
                    eval_adapter = T2IAdapter(cin=int(64 * 4), 
                         channels=[320, 640, 1280, 1280][:4], 
                         nums_rb=2, 
                         ksize=1, 
                         sk=True, 
                    ...
                    if args.use_ema:
                        ema_model.copy_to(get_trainable_parameters(eval_adapter))
