...
from src.pipelines.pipeline_inpainting import (
...
def main():
    ...
    if args.load_pretrained_weights != None:
        ...
        # NOTE Inpainting Stage1: "unet" -> "unet_Inapinting"
    ...
    # NOTE Inpainting Stage1: LoRA
    # Set correct lora layers
    # from diffusers.models.attention_processor import LoRAAttnProcessor
    # from diffusers.loaders import AttnProcsLayers
    lora_attn_procs = {}
    # 版本过低：should be removed since diffusers' version 0.26.3 is >= 0.26.0
    ...
    # accelerator.register_for_checkpointing(unet_lora_config.layers_to_transform)
    '''NOTE Inpainting Stage1: LoRA'''
    ...
    def unwrap_model(model):
        ...
        if args.use_ema:
            ema_model.copy_to(get_trainable_parameters(model))
        model = model._orig_mod if is_compiled_module(model) else model
        return model


    # NOTE Inpainting Stage1: unet
    model = unet
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                with accelerator.autocast():
                    ...
                    if args.mode == "normal":
                        ...
                        cond_latents = torch.cat([shadow_mask, comp_latents], dim=1)
                    else:
                    ...
                    if args.nofg:
                        ...
                        # NOTE Inpainting Stage1: change all ‘mask’ to ‘shadow_mask’
                        loss = F.mse_loss(
                            model_pred.float() * shadow_mask,
                            target.float() * shadow_mask,
                            reduction="none",
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step % args.checkpointing_steps == 0) or (
                    ...
                    if accelerator.is_main_process:
                        '''NOTE Inpainting Stage1: save lora weight XXX BAGIN XXX'''
                        ...
                        #     )
                        '''NOTE Inpainting Stage1: save weight XXX END XXX'''
                ...
                if (
                    ...
                    '''NOTE Inpainting Stage1: LoRA BEGIN'''
                    from peft import LoraConfig

                    # now we will add new LoRA weights to the attention layers
                    eval_unet_lora_config = LoraConfig(
                        r=args.rank,
                        lora_alpha=args.rank,
                        init_lora_weights="gaussian",
                        target_modules=["to_k", "to_q", "to_v", "to_out.0"],
                    )
                    eval_unet.add_adapter(eval_unet_lora_config)
                    '''NOTE Inpainting Stage1: LoRA END'''
                    ...
                    # NOTE Inpainting Stage1: add lora
                    # pipeline.unet.load_attn_procs(model_save_dir)
